...
import {MessageNewEvent, CallEndedEvent, CallRecordingReadyEvent, CallSessionParticipantLeftEvent, CallSessionStartedEvent, CallTranscription, CallTranscriptionReadyEvent } from "@stream-io/node-sdk";
...
export async function POST(req:NextRequest){
    ...
    try{
        ...
        try {
           ...
           await call.end()  
            ...
            if(!meetingId){
                return NextResponse.json({error: "missinng meeting id "}, {status: 400});
            }
    ...
    if (userId !== existingAgent.id) {
      ...
      const avatarUrl = genetratedAvatarUri({
