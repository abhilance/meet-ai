...
import { meetingsInsertionSchema, meetingsUpdateSchema } from "../schema"
      ...
      const userSpeakers = await db
        ...
        .then((users) =>
          users.map((user) => ({
            ...
            image:
              user.image ??
              genetratedAvatarUri({ seed: user.name, variant: "initials" }),
          }))
        );
    ...
    await streamVideo.upsertUsers([
      {
        ...
        image:
        ctx.auth.user.image ?? 
        genetratedAvatarUri({seed : ctx.auth.user.name, variant:"initials"}),
      },
    ])
